...
func (r *mutationResolver) MetadataCleanGenerated(ctx context.Context, input task.CleanGeneratedOptions) (string, error) {
	...
	t := &task.CleanGeneratedJob{
		...
		PrefixedThumbnailDB:      mgr.PrefixedThumbnailDB,
		PerGalleryThumbnailDB:    mgr.PerGalleryThumbnailDB,
		HybridThumbnailDB:        mgr.HybridThumbnailDB,
	}
...
func (r *mutationResolver) MigrateThumbnails(ctx context.Context) (string, error) {
	jobID := manager.GetInstance().MigrateThumbnails(ctx, false)
	return strconv.Itoa(jobID), nil
}
